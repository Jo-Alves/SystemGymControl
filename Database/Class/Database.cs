...
namespace Database
{
    public class database
    {
        public static bool ExistsDatabase()
        {
            ...
            {
                string _sql = "Select * from Sys.Databases where name = 'dbGymControl'";
        ...
        public static void CreateDatabase()
        {
            using (var connection = new SqlConnection(ConnectionDataBase.stringConnectionMaster))
            {
                try
                {
                    ...
                    command.CommandText = "CREATE DATABASE dbGymControl";
        ...
        public static void CreateTables()
        {
            using (var connection = new SqlConnection(ConnectionDataBase.stringConnection))
            {
                try
                {
                    connection.Open();
                    var command = new SqlCommand("", connection);
                    command.CommandText = "CREATE TABLE [dbo].[students]" +
                        "(" +
                        ...
                        "	refering VARCHAR(MAX) NULL," +
                        "	document_number INT NULL," +
                        "	due_date VARCHAR(10) NULL," +
                        "	document_value DECIMAL(18, 2) NULL," +
