        ...
        public override void Save()
        {
            using (var connection = new SqlConnection(ConnectionDataBase.stringConnection))
            {
                if (_id == 0)
                    ...
                    _sql = "UPDATE students SET name = @name, cpf = @cpf, phone = @phone, cep = @cep, district = @district, address = @address, number = @number, birth = @birth, city = @city, state = @state, email = @email, photo = @photo WHERE id = @id";
